...
- **Síntoma:** Un usuario conoce el `_id` de la orden ajena y `GET /orders/:id` devuelve 200 con datos ajenos.
- **Causa raíz:** `findById(orderId)` no filtra por `userId` del header `x-user-id`.
- **Fix:** Cambiar a `findOne({ _id: orderId, userId })` y responder 404 si no hay match.
- **Prevención:** Test e2e de IDOR (ya incluido) + siempre filtrar ownership en la query, no en memoria.
```
...
```bash
git commit -m "fix(infra): docker build multi-stage y MONGO_URI desde env"
git commit -m "fix(validation): ValidationPipe global y DTOs con class-validator"
git commit -m "fix(security): filtrar órdenes por userId (IDOR)"
git commit -m "fix(orders): pago atómico con transacción MongoDB"
git commit -m "fix(reconciliation): cron idempotente cada 5 min"
git commit -m "perf(orders): batch query de productos e índices en schemas"
git commit -m "test: pago concurrente y expect 400 en oversell"
git commit -m "docs: completar BUGS.md"
```

---

## Anexo — `orders.service.ts` recomendado (archivo completo)

Referencia unificada de cómo quedaría el servicio tras aplicar los retos 2.3–2.6 y 3.1:
...
- [ ] `docker compose up --build` → API en `:3000`
- [ ] `npm test` → todos los tests en verde (incl. concurrencia propio)
- [ ] `BUGS.md` completo (mín. 8–10 entradas)
- [ ] Repo público en GitHub con `.env.example` actualizado
- [ ] Preparar defensa oral: **por qué transacciones** y **por qué 404 en IDOR** (no 403)
...
*Generado como guía de implementación. Ajustar según hallazgos al ejecutar `npm test` y `docker compose up` en tu entorno local.*